...
\mentor{dr Milena \textsc{Vujošević Janičić}, neredovan profesor\\ Univerzitet u Beogradu, Matematički fakultet}
...
jeziku \textit{Rust}. Biblioteke napisane u \textit{Rust}-u
...
nazvanu \texttt{insert\_or\_update}, koja umeće dati par
...
steka. Funkcija \texttt{find} je \textit{Rust} ekvivalent funkciji
...
Funkcija \texttt{find\_child} je pandan funkciji \texttt{findChild}
sa listinga~\ref{kod:algoritam_findChild}. Međutim, usled
pravila pozajmljivanja, ova funkcija ima svog mutabilnog dvojnika
...
Implementacija funkcije \texttt{insert\_or\_update} je složenija
...
\begin{enumerate}
  \item Nalaženje poslednjeg
        čvora koji se poklapa za zadatim ključem.

  \item Dodavanje para na mesto nađeno u prvom koraku.
\end{enumerate}

Prvi korak je implementiran u vidu petlje koja se kreće od korena
...
Ovo znači da se u petlju ne ulazi ako je stablo prazno.
Pretraga se prekida kada se dođe do lista ili pretraga u unutrašnjem
čvoru vodi ka nedostajućoj vrednost.
Takođe, u ovom koraku se proverava da li je pretraga došla do kraja,
odnosno, da li je neophodno uneti vrednost u trenutni unutrašnji čvor
i vratiti staru vrednost, ako ona postoji.

Drugi korak zavisi od toga gde se prekinula pretraga u prvom koraku.
Ukoliko je u pitanju nedostajuća vrednost, stablo je prazno i nedostajuću
vrednost treba zameniti listom koji sadrži dati par.
Ukoliko je u pitanju list, proverava se ključ pretrage i ključ u tom listu.
Ako dođe do potpunog poklapanja, potrebno
je ažurirati vrednost u listu i vratiti staru vrednost.
U suprotnom, ako je došlo do parcijalnog poklapanja ključa,
potrebno je napraviti novi unutrašnji čvor koji menja
stari list. Parcijalni ključ novog unutrašnjeg čvora
je deo ključa pretrage koji se poklapa sa ključem u listu.
U novonastali čvor treba dodati stari list kao i novi list,
...
parcijalnim ključem unutrašnjeg čvora.
U novonapravljeni čvor treba dodati stari čvor kao i list sa novim parom.
...
dosadašnjem stablu.
Tada je samo potrebno dodati list sa novim parom u unutrašnji
...
če se ta vrednost naći i izbrisati.
Ovaj par se ne ulazi u broj dece koji se čuva u unutrašnjem čvoru,
pa se broj dece ne koriguje prilikom brisanja vrednosti, niti
se čvor smanjuje.
