...
funkcija \texttt{find} kao \textit{Rust} ekvivalent funkciji
...
Prvi test je \texttt{insert\_update\_delete\_find}, prikazan na
...
\texttt{insert\_or\_update}, postojanje tih parova u samoj strukturi
se proverava funkcijom \texttt{find}. Sledi provera funkcionalnosti
...
\begin{lstlisting}[language=Rust,
                   caption={Test \texttt{insert\_update\_delete\_find}},
                   label={kod:iudf_test}]
#[test]
...
adekvatno tranformišu u nizove bajtova. Nakon unosa parova u ART,
...
listingu~\ref{kod:benches}. U funkciji \texttt{bench\_finds} se nalazi
...
pravi se nova grupa merenja sa nazivom \texttt{Finds}. U grupi testova
...
\begin{lstlisting}[language=Rust,
                   caption={Funkcija \texttt{bench\_finds}},
                   label={kod:benches}]
...
parova. Brojač se inkrementira za 200 hiljada u svakoj iteraciji do
granične vrednosti koja je podešena na 4 miliona.
...
korektna. Na listingu~\ref{kod:art_find} se nalazi jedna od pomenutih
pomoćnih funkcija, koja je
zadužena za pretragu unutar strukture ART.\ Sličnu funkcionalnost imaju i
funkcije \texttt{hmap\_find} i \texttt{btree\_find} koje pretragu vrše
unutar heš mape, odnosno B-stabla.

\begin{lstlisting}[language=Rust,
                   caption={Funkcija \texttt{art\_find}},
                   label={kod:art_find}]
...
Na slici~\ref{fig:bench_find} se nalaze rezultati merenja operacije
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.90\textwidth]{lines_find.eps}
  \caption{
